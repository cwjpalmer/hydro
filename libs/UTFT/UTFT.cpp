...
void UTFT::setScaleFactor(float x, float y)
...
void UTFT::drawRoundRect(int x1, int y1, int x2, int y2)
{
	...
	if ((x2-x1)>4 && (y2-y1)>4)
	{
		drawPixel((x1+1)/scale_factor_x,(y1+1)/scale_factor_y);
		drawPixel((x2-1)/scale_factor_x,(y1+1)/scale_factor_y);
		drawPixel((x1+1)/scale_factor_x,(y2-1)/scale_factor_y);
		drawPixel((x2-1)/scale_factor_x,(y2-1)/scale_factor_y);
...
void UTFT::print(char *st, int x, int y, int deg)
{
    ...
    {
        x=((disp_x_size+1)*scale_factor_x)-(stl*cfont.x_size);
    }
	if (x==CENTER)
		{
		    x=(((disp_x_size+1)*scale_factor_x)-(stl*cfont.x_size))/2;
		}
	}
	else
	{
